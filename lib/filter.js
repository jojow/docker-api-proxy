...
var reContUpdate = new RegExp('.*/containers/.*/update.*', 'i');
var reVolumes = new RegExp('.*/volumes.*', 'i');
var reNetworks = new RegExp('.*/networks.*', 'i');
var reSwarm = new RegExp('.*/swarm.*', 'i');
var reServices = new RegExp('.*/services.*', 'i');
...
var allowRequest = function(req) {
  ...
  if (reqMethod === 'post') {
    ...
    if (reContUpdate.test(req.url)) return false;
    if (reVolumes.test(req.url)) return false;
    if (reNetworks.test(req.url)) return false;
    if (reSwarm.test(req.url)) return false;
    if (reServices.test(req.url)) return false;
  ...
  if (reqMethod === 'get') {
    ...
    if (reContExport.test(req.url)) return false;
