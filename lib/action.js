...
var reContKill = new RegExp('.*/containers/.*/kill.*', 'i');
...
var manageGateway = function(req, res, ctx, callback) {
  ...
  res.on('finish', function() {
    ...
    } else if (reContStop.test(req.url) || reContKill.test(req.url)) {
      var urlParts = req.url.split('/');
      urlParts.pop();
      var containerId = urlParts.pop();

      _.each(childProcesses, function(child, port) {
        if (_.startsWith(child.id, containerId) || _.startsWith(containerId, child.id)) child.proc.kill('SIGKILL');
